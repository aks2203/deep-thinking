...
def create_data():
    ...
    givens_max = 34
    train = generate(train_pool, givens_max, 900)
    valid = generate(valid_pool, givens_max, 1000)
    givens_max = 28
    test = generate(test_pool, givens_max, 10000)

    dump('train_2.csv', train)
    dump('valid_2.csv', valid)
    dump('test_2.csv', test)
...
def prepare_sudoku_2_loader(train_batch_size, test_batch_size, train_data, test_data, shuffle=True):

    create_data()

    trainset = dataset(root_dir = './', dataset_type='train_2')
    valset = dataset(root_dir='./', dataset_type='valid_2')
    testset = dataset(root_dir='./', dataset_type='test_2')
