...
def create_data():
    ...
    dump('train_5.csv', train)
    dump('valid_5.csv', valid)
    dump('test_5.csv', test)
...
def prepare_sudoku_5_loader(train_batch_size, test_batch_size, train_data, test_data, shuffle=True):

    #create_data()

    trainset = dataset(root_dir = './', dataset_type='train_5')
    valset = dataset(root_dir='./', dataset_type='valid_5')
    testset = dataset(root_dir='./', dataset_type='test_5')
