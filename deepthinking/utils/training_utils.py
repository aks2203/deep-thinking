...
def get_output_for_random2_loss(inputs, max_iters, net):
    # get features from n iterations to use as input
    n = randrange(0, max_iters)
    ...
    outputs, _ = net(inputs, iters_elapsed=0, iters_to_do=k, interim_thought=interim_thought)
    return outputs


def get_output_for_random_loss(inputs, max_iters, net):
    # get features from n iterations to use as input
    n = 0
...
def train(net, loaders, mode, train_setup, device, disable_tqdm=False):
    if mode == "progressive":
        ...
        train_loss, acc = train_random(net, loaders, train_setup, device, disable_tqdm)
    elif mode == "every_iter":
        train_loss, acc = train_every_iter(net, loaders, train_setup, device, disable_tqdm)
    elif mode == "random_2":
...
def train_random(net, loaders, train_setup, device, disable_tqdm):
    ...
    for batch_idx, (inputs, targets) in enumerate(tqdm(trainloader, leave=False,
        ...
        # so we save time by setting it equal to 0).
        if alpha != 0:
            outputs = get_output_for_random_loss(inputs, max_iters, net)
            outputs = outputs.view(outputs.size(0), outputs.size(1), -1)
            loss_progressive = criterion(outputs, targets)
        else:
            loss_progressive = torch.zeros_like(targets).float()
...
def train_random_2(net, loaders, train_setup, device, disable_tqdm):
    ...
    for batch_idx, (inputs, targets) in enumerate(tqdm(trainloader, leave=False,
        ...
        # so we save time by setting it equal to 0).
        if alpha != 0:
            outputs = get_output_for_random2_loss(inputs, max_iters, net)
            outputs = outputs.view(outputs.size(0), outputs.size(1), -1)
            loss_progressive = criterion(outputs, targets)
        else:
            loss_progressive = torch.zeros_like(targets).float()
...
def train_every_iter(net, loaders, train_setup, device, disable_tqdm):
    ...
    for batch_idx, (inputs, targets) in enumerate(tqdm(trainloader, leave=False,
        ...
        for iters in range(max_iters):
            outputs_max_iters = outputs[:, iters]
            outputs_max_iters = outputs_max_iters.view(outputs_max_iters.size(0),
                                                       outputs_max_iters.size(1), -1)
            loss_iters += criterion(outputs_max_iters, targets)

        if problem == "mazes":
            loss_iters = (loss_iters * mask)
            loss_iters = loss_iters[mask > 0]

        loss = loss_iters.mean()
        loss.backward()
...
def train_weight_every_iter(net, loaders, train_setup, device, disable_tqdm):
    ...
    for batch_idx, (inputs, targets) in enumerate(tqdm(trainloader, leave=False,
        ...
        for iters in range(max_iters):
            ...
            outputs_max_iters = outputs_max_iters.view(outputs_max_iters.size(0),
                                                       outputs_max_iters.size(1), -1)
            loss_iters += (iters+1)*criterion(outputs_max_iters, targets)
